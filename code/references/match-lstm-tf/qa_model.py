...
class Encoder(object):
    ...
    def encode(self, inputs, masks, encoder_state_input=None):
        ...
        with tf.variable_scope("encoded_passage"):
            lstm_cell_passage = tf.contrib.rnn.BasicLSTMCell(
                self.hidden_size, state_is_tuple=True)
            encoded_passage, (p_rep, _) = tf.nn.dynamic_rnn(
                lstm_cell_passage, passage, masks_passage, dtype=tf.float32)  # (-1, P, H)
...
class BaselineDecoder(object):
    ...
    def decode(self, encoded_passage, q_rep, mask, labels):
        ...
        def func(score): return _maybe_mask_score(score, mask, float("-inf"))
...
class Decoder(object):
    ...
    def run_lstm(self, encoded_rep, q_rep, masks):
        ...
        mixed_question_passage_rep = tf.concat(
            [encoded_passage, q_rep], axis=-1)

        with tf.variable_scope("lstm_"):
            cell = tf.contrib.rnn.BasicLSTMCell(
                self.hidden_size, state_is_tuple=True)
            reverse_mixed_question_passage_rep = _reverse(
                mixed_question_passage_rep, masks_passage, 1, 0)

            output_attender_fw, _ = tf.nn.dynamic_rnn(
                cell, mixed_question_passage_rep, dtype=tf.float32, scope="rnn")
            output_attender_bw, _ = tf.nn.dynamic_rnn(
                cell, reverse_mixed_question_passage_rep, dtype=tf.float32, scope="rnn")

            output_attender_bw = _reverse(
                output_attender_bw, masks_passage, 1, 0)
    ...
    def run_match_lstm(self, encoded_rep, masks):
        ...
        def match_lstm_cell_attention_fn(curr_input, state): return tf.concat([
        ...
        with tf.variable_scope("match_lstm_attender"):
            ...
            lstm_attender = AttentionWrapper(
                cell, attention_mechanism_match_lstm, output_attention=False, attention_input_fn=match_lstm_cell_attention_fn)

            # we don't mask the passage because masking the memories will be handled by the pointerNet
            reverse_encoded_passage = _reverse(
                encoded_passage, masks_passage, 1, 0)

            output_attender_fw, _ = tf.nn.dynamic_rnn(
                lstm_attender, encoded_passage, dtype=tf.float32, scope="rnn")
            output_attender_bw, _ = tf.nn.dynamic_rnn(
    ...
    def run_answer_ptr(self, output_attender, masks, labels):
        ...
        def answer_ptr_cell_input_fn(
            curr_input, context): return context  # independent of question
        ...
        with tf.variable_scope("answer_ptr_attender"):
            attention_mechanism_answer_ptr = BahdanauAttention(
                query_depth_answer_ptr, output_attender, memory_sequence_length=masks_passage)
            ...
            answer_ptr_attender = AttentionWrapper(
                cell_answer_ptr, attention_mechanism_answer_ptr, cell_input_fn=answer_ptr_cell_input_fn)
...
class QASystem(object):
    ...
    def setup_word_embeddings(self):
        ...
        with tf.variable_scope("vocab_embeddings"):
            _word_embeddings = tf.Variable(
                self.embeddings, name="_word_embeddings", dtype=tf.float32, trainable=self.config.train_embeddings)
            question_emb = tf.nn.embedding_lookup(
                _word_embeddings, self.question_ids, name="question")  # (-1, Q, D)
            passage_emb = tf.nn.embedding_lookup(
                _word_embeddings, self.passage_ids, name="passage")  # (-1, P, D)
            # Apply dropout
    ...
    def setup_placeholders(self):
        self.question_ids = tf.placeholder(
            tf.int32, shape=[None, None], name="question_ids")
        self.passage_ids = tf.placeholder(
            tf.int32, shape=[None, None], name="passage_ids")

        self.question_lengths = tf.placeholder(
            tf.int32, shape=[None], name="question_lengths")
        self.passage_lengths = tf.placeholder(
            tf.int32, shape=[None], name="passage_lengths")

        self.labels = tf.placeholder(
            tf.int32, shape=[None, 2], name="gold_labels")
    ...
    def setup_system(self):
        ...
        if self.config.use_match:
            self.logger.info("\n========Using Match LSTM=========\n")
            logits = decoder.decode([encoded_question, encoded_passage], q_rep, [
                                    self.question_lengths, self.passage_lengths], self.labels)
        else:
            self.logger.info("\n========Using Vanilla LSTM=========\n")
            logits = decoder.decode_lstm([encoded_question, encoded_passage], q_rep, [
                                         self.question_lengths, self.passage_lengths], self.labels)
    ...
    def initialize_model(self, session, train_dir):
        ...
        if ckpt and (tf.gfile.Exists(ckpt.model_checkpoint_path) or tf.gfile.Exists(v2_path)):
            self.logger.info("Reading model parameters from %s" %
                             ckpt.model_checkpoint_path)
            self.saver.restore(session, ckpt.model_checkpoint_path)
        else:
            self.logger.info("Created model with fresh parameters.")
            session.run(self.init)
            self.logger.info('Num params: %d' % sum(
    ...
    def run_epoch(self, session, train):
        ...
        nbatches = (len(train) + self.config.batch_size - 1) / \
        ...
        for i, (q_batch, c_batch, a_batch) in enumerate(minibatches(train, self.config.batch_size)):
            ...
            input_feed = self.get_feed_dict(
                q_batch, c_batch, a_batch, self.config.dropout_val)

            _, train_loss = session.run(
                [self.train_op, self.loss], feed_dict=input_feed)
