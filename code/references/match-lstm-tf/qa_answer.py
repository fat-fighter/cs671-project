    ...
    for articles_id in tqdm(range(len(dataset['data'])), desc="Preprocessing {}".format(tier)):
        ...
        for pid in range(len(article_paragraphs)):
            ...
            for qid in range(len(qas)):
                ...
                context_ids = [str(vocab.get(w, qa_data.UNK_ID))
                               for w in context_tokens]
                qustion_ids = [str(vocab.get(w, qa_data.UNK_ID))
...
def prepare_dev2(config):
    dev = squad_dataset(config.question_dev,
                        config.context_dev, config.answer_dev)
...
def run_func2(dataset, config):
    ...
    with tf.Session() as sess:
        ...
        answers, answers_canonical = generate_answers(
            sess, qa, dataset, question_uuid_data, rev_vocab)
