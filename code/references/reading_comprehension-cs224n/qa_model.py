...
class Encoder(object):
    ...
    def encode(self, context, context_m, question, question_m, embedding, keep_prob):
        ...
        with tf.variable_scope('context'):
            con_lstm_fw_cell = rnn.BasicLSTMCell(
                cfg.lstm_num_hidden, forget_bias=1.0)
            con_lstm_bw_cell = rnn.BasicLSTMCell(
                cfg.lstm_num_hidden, forget_bias=1.0)
        ...
        with tf.variable_scope('question'):
            ques_lstm_fw_cell = rnn.BasicLSTMCell(
                cfg.lstm_num_hidden, forget_bias=1.0)
            ques_lstm_bw_cell = rnn.BasicLSTMCell(
                cfg.lstm_num_hidden, forget_bias=1.0)
...
class Decoder(object):
    ...
    def decode(self, H_r, context_m, keep_prob):
        ...
        Hr_attend = tf.reduce_sum(tf.multiply(
            H_r, tf.expand_dims(s_prob, axis=[2])), axis=1)
...
class QASystem(object):
    def __init__(self, encoder, decoder, embed_path):
        ...
        self.keep_prob = tf.placeholder(
            dtype=tf.float32, name="dropout", shape=())
        ...
        with tf.variable_scope("qa",
            ...
            self.starter_learning_rate = tf.placeholder(
                tf.float32, name='start_lr')
            ...
            capped_gvs = [(tf.clip_by_value(grad, -cfg.clip_by_val,
                                            cfg.clip_by_val), var) for grad, var in gradients]
    ...
    def setup_system(self):
        ...
        self.s_score, self.e_score = self.decoder.decode(
            H_r, self.context_m, self.keep_prob)
    ...
    def setup_embeddings(self):
        ...
        logging.info('embed size: {} for path {}'.format(
            cfg.embed_size, self.embed_path))
        self.embedding = np.load(self.embed_path)['glove']
        self.embedding = tf.Variable(
            self.embedding, dtype=tf.float32, trainable=False)
    ...
    def optimize(self, session, context, question, answer, lr):
        ...
        output_feed = [self.merged, self.train_op,
                       self.final_loss, self.grad_norm]
            ...
            if sendin and len(sendin) > 2:
                ...
                for i in tqdm(range(train_len // input_batch_size), desc='trianing set'):
                    ...
                    train_as, train_ae = self.answer(session,
                                                     train_context[i * input_batch_size:(
                                                         i + 1) * input_batch_size],
    ...
    def train(self, lr, session, dataset, answers, train_dir, debug_num=0, raw_answers=None,
        ...
        if debug_num:
            assert isinstance(
                debug_num, int), 'the debug number should be a integer'
        ...
        for ep in xrange(self.epochs):
            ...
            logging.info(
                'training epoch ---- {}/{} -----'.format(ep + 1, self.epochs))
            ep_loss = 0.
            for it in xrange(batch_num):
                if not cfg.valohai:
                    sys.stdout.write('> %d / %d \r' %
                                     (self.iters % print_every, print_every))
                    sys.stdout.flush()
                context = train_context[it * batch_size: (it + 1) * batch_size]
                question = train_question[it *
                                          batch_size: (it + 1) * batch_size]
                ...
                if self.iters % print_every == 0:
                    ...
                    if cfg.valohai:
                        print(json.dumps({'iters': self.iters, 'loss': loss.item(
                        ), 'tf1': tf1, 'tem': tem, 'f1': f1, 'em': em}))
            ...
            logging.info('average loss of epoch {}/{} is {}'.format(ep +
                                                                    1, self.epochs, ep_loss / batch_num))
            ...
            data_save_path = pjoin(cfg.cache_dir, str(
                self.iters) + 'iters' + c_time + '.npz')
